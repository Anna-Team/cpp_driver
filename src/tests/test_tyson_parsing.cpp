...
TEST(tyson_parsing, insert_to_empty_map)
{
    std::stringstream sstream;
    
    auto map = tyson::TySonObject::Map();
    ASSERT_TRUE(map.emplace("foo", tyson::TySonObject::Number(10)));
    ASSERT_FALSE(map.emplace("foo", tyson::TySonObject::String("bar")));
    
    sstream << map;
    ASSERT_EQ(sstream.str(), "m{s|foo|:n|10|,}");
}

TEST(tyson_parsing, emplace_can_only_be_used_with_maps)
{
    std::stringstream sstream;
    
    auto tyson_vector = tyson::TySonObject::Vector(
            tyson::TySonObject::Number(10),
            tyson::TySonObject::String("bar"));
    
    ASSERT_THROW(tyson::TySonObject::Number(10).emplace("Allowed", tyson::TySonObject::Bool(false)),
                 std::invalid_argument);
    ASSERT_THROW(tyson::TySonObject::String("bar").emplace("Allowed", tyson::TySonObject::Bool(false)),
                 std::invalid_argument);
    ASSERT_THROW(tyson_vector.emplace("Allowed", tyson::TySonObject::Bool(false)), std::invalid_argument);
}
